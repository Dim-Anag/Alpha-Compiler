...
vardecl: type IDENT {
            ...
            Symbol *s = insert_symbol(symbol_table, $2, VARIABLE, @2.first_line, current_scope);
          //  printf("DEBUG: insert_symbol name=%s, type=%d, addr=%p\n", s->name, s->type, (void*)s);
assert(s->type == VARIABLE || s->type == FORMAL_ARGUMENT || s->type == USER_FUNCTION || s->type == LIBRARY_FUNCTION);
         }
...
expr:  KEYWORD_LOCAL IDENT OPERATOR_EQUAL expr {
      // Declare a new local variable and assign a value
        ...
        Symbol *s = insert_symbol(symbol_table, $2, VARIABLE, yylineno, current_scope);
      //  printf("DEBUG: insert_symbol name=%s, type=%d, addr=%p\n", s->name, s->type, (void*)s);
        assert(s->type == VARIABLE || s->type == FORMAL_ARGUMENT || s->type == USER_FUNCTION || s->type == LIBRARY_FUNCTION);
...
| expr OPERATOR_PLUS expr {
    ...
    Symbol *t = newtemp(symbol_table, current_scope); 
    temp->sym = t;
  //  printf("DEBUG: newtemp name=%s, type=%d, addr=%p\n", t->name, t->type, (void*)t);
    assert(t->type == VARIABLE || t->type == TEMPVAR); 
...
| lvalue OPERATOR_INCREMENT %prec OPERATOR_INCREMENT {
    ...
    if ($1->type == tableitem_e) {
        ...
        // old value first
        Symbol *t1 = newtemp(symbol_table, current_scope);
        expr* oldval = newexpr(var_e); oldval->sym = t1;
      //  printf("DEBUG: newtemp name=%s, type=%d, addr=%p\n", t1->name, t1->type, (void*)t1);
        assert(t1->type == VARIABLE); 
        emit(tablegetelem, oldval, table, $1->index, 0, yylineno);

        // krata old value se temp gia expr
        Symbol *t2 = newtemp(symbol_table, current_scope);
        expr* temp = newexpr(var_e); temp->sym = t2;
      //  printf("DEBUG: newtemp name=%s, type=%d, addr=%p\n", t2->name, t2->type, (void*)t2);
        assert(t2->type == VARIABLE); 
        emit(assign, temp, oldval, NULL, 0, yylineno);

        // kanei prwta assign meta add
        Symbol *t3 = newtemp(symbol_table, current_scope);
        expr* newval = newexpr(var_e); newval->sym = t3;
      //  printf("DEBUG: newtemp name=%s, type=%d, addr=%p\n", t3->name, t3->type, (void*)t3);
        assert(t3->type == VARIABLE); 
        ...
        Symbol *t = newtemp(symbol_table, current_scope);
        expr* temp = newexpr(var_e); temp->sym = t;
      //  printf("DEBUG: newtemp name=%s, type=%d, addr=%p\n", t->name, t->type, (void*)t);
        assert(t->type == VARIABLE || t->type == TEMPVAR); 
       ...
       // printf("DEBUG: newtemp name=%s, type=%d, addr=%p\n", t->name, t->type, (void*)t);
        ...
        if (!symbol) {
            if (lval_expr->sym->type != MEMBER) {
                Symbol *s = insert_symbol(symbol_table, lval_expr->sym->name, VARIABLE, yylineno, current_scope);
             //   printf("DEBUG: insert_symbol name=%s, type=%d, addr=%p\n", s->name, s->type, (void*)s);
                assert(s->type == VARIABLE || s->type == FORMAL_ARGUMENT || s->type == USER_FUNCTION || s->type == LIBRARY_FUNCTION);
            }
...
primary
        : lvalue %prec PUNCTUATION_DOUBLE_DOT { 
            $$ = emit_iftableitem($1);
          //  printf("primary: expr type = %d, name = %s\n", $$ ? $$->type : -1, ($$ && $$->sym) ? $$->sym->name : "(null)"); 
            }
       | call {
         $$ = $1;
         //  printf("primary: expr type = %d, name = %s\n", $$ ? $$->type : -1, ($$ && $$->sym) ? $$->sym->name : "(null)");
...
lvalue: IDENT %prec PUNCTUATION_DOUBLE_DOT {
    // Debug: print all active 'print' symbols in all scopes
    for (int s = current_scope; s >= 0; --s) {
        Symbol *sym = lookup_symbol(symbol_table, "print", s, 1);
        if (sym) {
          //  printf("DEBUG: Found symbol 'print' in scope %d, type %d\n", s, sym->type);
        }
    }

    Symbol *symbol = lookup_symbol(symbol_table, $1, current_scope, 0);
    if (!symbol) {
        // Check global scope for library function
        symbol = lookup_symbol(symbol_table, $1, 0, 1);
    }
    if (!symbol) {
      Symbol *s =  insert_symbol(symbol_table, $1, VARIABLE, yylineno, current_scope);
       // printf("DEBUG: insert_symbol name=%s, type=%d, addr=%p\n", s->name, s->type, (void*)s);
assert(s->type == VARIABLE || s->type == FORMAL_ARGUMENT || s->type == USER_FUNCTION || s->type == LIBRARY_FUNCTION);
    ...
    expr* e;
    if (symbol->type == LIBRARY_FUNCTION) {
    ...
    e->sym = symbol;
    $$ = e;
   // printf("lvalue IDENT: symbol type = %d, expr type = %d, name = %s\n", symbol->type, e->type, symbol->name);
}
| KEYWORD_LOCAL IDENT {
    Symbol *existing = lookup_symbol(symbol_table, $2, current_scope, 1);
    if (existing) {
...
call
    : primary PUNCTUATION_LEFT_PARENTHESIS elist PUNCTUATION_RIGHT_PARENTHESIS {
        ...
        if (!func || !func->sym) {
            printf("ERROR: call rule got invalid func expression at line %d\n", yylineno);
        } else {
          //  printf("call rule: func expr type = %d, name = %s\n", func->type, func->sym->name);
        }
        ...
        for (i = 0; i < count; i++) {
          //  printf("emit param: expr type = %d, name = %s\n",
             //   params[i] ? params[i]->type : -1,
              //  (params[i] && params[i]->sym) ? params[i]->sym->name : "(null)");
            emit(param, NULL, params[i], NULL, 0, yylineno);
        }
        free(params);
      ...
      //  printf("DEBUG: newtemp name=%s, type=%d, addr=%p\n", t->name, t->type, (void*)t);
        assert(t->type == VARIABLE || t->type == TEMPVAR); 
     //   printf("DEBUG: BEFORE emit getretval, temp name=%s, type=%d, addr=%p\n", temp->sym->name, temp->sym->type, (void*)temp->sym);

        emit(getretval, temp, NULL, NULL, 0, yylineno);
      //  printf("DEBUG: AFTER emit getretval, temp name=%s, type=%d, addr=%p\n", temp->sym->name, temp->sym->type, (void*)temp->sym);
...
expr_list
    : expr {
        if ($1) {
          //  printf("expr_list: expr type = %d, name = %s\n",
          //      $1->type,
          //      ($1->sym) ? $1->sym->name : "(null)");
            exprlist* l = malloc(sizeof(exprlist));
            l->expr = $1; l->next = NULL;
            $$ = l;
        } else {
            printf("WARNING: expr_list got NULL expr, skipping!\n");
            $$ = NULL;
        }
    }
...
funcdecl_head: KEYWORD_FUNCTION IDENT PUNCTUATION_LEFT_PARENTHESIS {
  ...
  //  printf("DEBUG: insert_symbol name=%s, type=%d, addr=%p\n", funcsym->name, funcsym->type, (void*)funcsym);
    assert(funcsym->type == VARIABLE || funcsym->type == FORMAL_ARGUMENT || funcsym->type == USER_FUNCTION || funcsym->type == LIBRARY_FUNCTION);
...
anonfuncdecl_head: KEYWORD_FUNCTION PUNCTUATION_LEFT_PARENTHESIS {
   ...
   Symbol *s = insert_symbol(symbol_table, anon_name, USER_FUNCTION, yylineno, current_scope);
  //  printf("DEBUG: insert_symbol name=%s, type=%d, addr=%p\n", s->name, s->type, (void*)s);
assert(s->type == VARIABLE || s->type == FORMAL_ARGUMENT || s->type == USER_FUNCTION || s->type == LIBRARY_FUNCTION);
    ...
    push(scopeoffsetstack, currscopeoffset());
    enterscopespace();           // Είσοδος στα formals
    resetformalargsoffset();     // Reset offset για τα formals
...
idlist: /* empty */
      | idlist PUNCTUATION_COMMA IDENT {
            ...
            int argument_scope = current_scope; 
         ...
         Symbol *s =   insert_symbol(symbol_table, $3, FORMAL_ARGUMENT, yylineno, argument_scope);
    //  printf("DEBUG: insert_symbol name=%s, type=%d, addr=%p\n", s->name, s->type, (void*)s);
assert(s->type == VARIABLE || s->type == FORMAL_ARGUMENT || s->type == USER_FUNCTION || s->type == LIBRARY_FUNCTION);
        }
      | IDENT {
            ...
            // Insert the formal argument into the symbol table
           Symbol *s = insert_symbol(symbol_table, $1, FORMAL_ARGUMENT, yylineno, argument_scope);
      // printf("DEBUG: insert_symbol name=%s, type=%d, addr=%p\n", s->name, s->type, (void*)s);
assert(s->type == VARIABLE || s->type == FORMAL_ARGUMENT || s->type == USER_FUNCTION || s->type == LIBRARY_FUNCTION);
        };
...
int main(int argc, char **argv) {
    ...
    // === AVM code ===
   // generate_final_code("program.abc"); 
       printf("DEBUG: About to call final code program txt\n");
    generate_final_code("program.txt");
        printf("DEBUG: end with final code\n");
           print_all_symbol_offsets(symbol_table);
            printf("DEBUG: About to call write_binary_output\n");


         

    printf("DEBUG: Before binary output:\n");




    write_binary_output("program.abc"); 

    free(instructions);
    free_const_tables();   


FILE* f = fopen("program.abc", "rb");
fseek(f, 0, SEEK_END);
long size = ftell(f);
fclose(f);
printf("DEBUG: program.abc size = %ld bytes\n", size);
