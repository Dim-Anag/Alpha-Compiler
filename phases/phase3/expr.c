...
int debug_mode = 1; // 0 για off, 1 για on
...
expr* to_bool(expr* e) {
    ...
    //lazy < > operators for the not keyword
    if (e->type == relop_e) {
    expr* left = e->left;
    expr* right = e->right;
    ...
    expr* bool_e = newexpr(boolexpr_e);
    emit(e->op, left, right, NULL, 0, yylineno);
    bool_e->truelist = makelist(nextquad() - 1);
    emit(jump, NULL, NULL, NULL, 0, yylineno);
    bool_e->falselist = makelist(nextquad() - 1);
    return bool_e;
...
expr* newexpr_programfunc_name(const char* name) {
    ...
    sym->line = yylineno;
    // Αν χρειάζεται, συμπλήρωσε και άλλα πεδία
...
expr* emit_iftableitem(expr* e) {
    ...
    result->sym = newtemp();
    // Τυλίγεις το sym σε expr για να το περάσεις στην emit
